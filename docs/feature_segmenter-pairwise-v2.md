...
### `BAAI/SeniorTalk` 授權（已查證，2026-07-27）

授權為 **CC BY-NC-SA 4.0**，另有 gated access 條款：僅限學術研究與教育用途、明文禁止商業使用
（含商業產品開發與任何形式的變現）、禁止再識別受訪者或抽取個資、衍生作品須以相同授權釋出。
來源：[HF dataset card](https://huggingface.co/datasets/BAAI/SeniorTalk)（內容已改寫以符合授權限制）。

對本專案的結論：

- **可以**用於 Test-Real 評測（非商業研究用途），前提是在 repo 標註來源與授權。
- **不可**把原始轉錄或其衍生標註集納入任何商業版本。本作品若走向商業化，Test-Real 必須改用
  自行錄製或取得商業授權的語料重建；`seniortalk_tw_balanced_corpus.jsonl` 已是衍生作品，同受 NC-SA 約束。
- ShareAlike 表示標註後的邊界資料集是衍生作品：**若要公開，須以 CC BY-NC-SA 4.0 釋出**；
  不公開則留在 repo 內部並標註授權即可。
- 因此 Test-Real 只作**評測**，不進訓練集，避免模型權重是否為衍生作品的爭議。artifact 不含原文。
