...
| 模型／服務 | 語言與腔調 | 授權／服務性質 | 預設狀態 |
|---|---|---|---|
| `formospeech/omnivoice-hakka-community-1` | 客語六腔 | CC BY-NC 4.0、gated | endpoint 與 production gate 關閉；客語主候選 |
| `formospeech/yourtts-htia-240704`（VoxHakka） | 四縣、海陸、大埔、饒平、詔安；預設 speaker `XF` | CC BY-NC 4.0 | endpoint 與 gate 關閉；同語言備援，不支援南四縣 |
| `MediaTek-Research/BreezyVoice` | 台灣華語候選 | Apache-2.0 | endpoint 與 gate 關閉；須先驗證繁體輸入、聲線、容量與 P95 |
