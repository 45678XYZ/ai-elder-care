...
- conversation action 明確區分 `create/update/deactivate/complete`，由對話大腦的 tool calling 在回應 chat Lambda 之前同步處理；batch 不得建立、修改、停用或完成 routine。`complete` 只寫 canonical event，不建立 routine 新版本。
