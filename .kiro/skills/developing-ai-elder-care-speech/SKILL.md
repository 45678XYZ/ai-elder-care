...
inclusion: fileMatch
fileMatchPattern: "**/{asr/**,tts/**,asr_*.tf,tts_*.tf,chat.py}"
---

# 語音子系統維護

本 skill 與專案 steering 一起使用。先讀本檔，再依修改範圍只讀下表列出的文件；
...
python -m pytest tests/asr/test_chat_asr_bridge.py tests/test_chat.py -q
python -m pytest tests/test_elders.py -q
```
