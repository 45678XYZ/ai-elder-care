...
# =============================================================================
# 模組 A：chat / tools / elders Lambda 資源宣告與 IAM 存取權限
# =============================================================================

# 1. 將 backend/src 目錄自動壓縮打包為 backend_src.zip
data "archive_file" "backend_src" {
  type        = "zip"
  source_dir  = "${path.module}/../backend/src"
  output_path = "${path.module}/build/backend_src.zip"
}

# 2. 建立模組 A 專屬 Lambda 共用 IAM Role (信任 Lambda 服務)
...
# 5. chat Lambda 函數 (POST /chat 對話進入點)
resource "aws_lambda_function" "chat" {
  ...
  filename         = data.archive_file.backend_src.output_path
  source_code_hash = data.archive_file.backend_src.output_base64sha256
...
# 6. tools Lambda 函數 (Bedrock Action Group 6 大工具箱)
...
# 7. elders Lambda 函數 (GET/POST/PATCH /elders 長者個人檔案與偏好 API)
