...
void main() {
  ...
  testWidgets('登出確認框的字也換得掉——最後補譯的那一組', (tester) async {
    ...
    // 逐句列出而不是 isEmpty：長者端一度全數譯完，但「刪掉自己加的行程」這條路
    // 是新加的，譯文要等 i18n/hakka-strings.md 那份清單回來（刪除不可逆，用詞
    // 猜錯的代價是長輩誤解按下去會發生什麼事，所以不在程式裡自己造）。
    //
    // 仍然斷言確切內容而不是 isNotEmpty：這樣新增畫面文字忘了送翻照樣會在這裡
    // 失敗，守門沒有鬆掉。補譯完把這幾句從 missingFromHakka 移進對照表，
    // 這裡改回 isEmpty。
    expect(missingFromHakka, [
      '我自己加的',
      '家人幫我排的',
      '刪掉這一項',
      '要刪掉「{}」嗎？\n刪掉以後就不會再提醒你了。',
