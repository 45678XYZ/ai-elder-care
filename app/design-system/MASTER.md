...
事件分類色（時間軸／摘要／統計，點色／字色／底色）。七類與 `docs/api.md` 的 `EventType`
一一對應；實作與唯一真實來源是 `lib/theme/app_theme.dart` 的 `EventCategoryStyle`，
下表是對照用，改色要兩邊一起改。

| 類別 | 點 | 字 | 底 | 色相 |
|---|---|---|---|---|
| 用藥 | `#c25d58` | `#7d281f` | `#ffe5e1` | 6° 紅 |
| 飲食 | `#a78100` | `#584200` | `#fbeec9` | 45° 琥珀 |
| 活動 | `#4d9351` | `#1f4e27` | `#dff6de` | 130° 綠 |
| 身心 | `#2e8296` | `#0e4a5c` | `#d9eff5` | 194° 青 |
| 睡眠 | `#676ba5` | `#453f6d` | `#eaedff` | 248° 靛 |
| 安全 | `#a63c82` | `#75205a` | `#f7e1f0` | 319° 紫紅 |
| 其他 | `#4f4335` | `#4f4335` | `#f2ece1` | 低彩度 |

字色一律對最深紙底 `#e4dccb` >=7:1；六個有彩度的類別色相兩兩至少差 35°，
用藥與身心曾共用同一個紅、安全曾用警示橘（夾在紅與琥珀之間），兩次都是畫面上分不出來，
現在由 `test/event_category_test.dart` 把不重複、對比與色相間距釘住。

圓點另分形狀（§9 顏色不得單用承載資訊）：飲食方形、安全菱形、其餘圓形。
