...
SYSTEM_PROMPT = (
    ...
    "長者家常閒聊、問候、談論天氣或個人看法（如「聊天」、「提到年輕人很忙」）屬於語言交流過程，嚴禁觸發標籤；"
    "只有伴隨實體外出/團體活動、實質求助或認知疑慮時才可標記社交標籤。"
)
...
7. 以下情況不要標記為命中：
   ...
   - 長者的詢問不等同於已發生的事實
8. 家常閒聊與泛對話行為過濾（最重要）：
   一般的問候、家常閒聊、討論新聞或個人看法（如「與鄰居聊天」、「提到年輕人很忙」）屬於語言交流過程，嚴禁觸發 InterpersonalSocialBehavior 或其他標籤。
   只有社交行為伴隨「實體外出/參加團體活動」（如去社區中心看戲）或「實質照護/求助」時才可標記。
