...
本套件把 WAV／M4A 正規化為 mono 16 kHz PCM，再依 `ASR_CONFIG_JSON` 呼叫已核准的
SageMaker endpoint。它不處理 HTTP、認證、session、資料庫或聊天流程。
...
| `config.py` | route、provider、model gate 與 `ASR_CONFIG_JSON` parser |
| `providers.py` | provider protocol、本機測試 mock 與 SageMaker adapter |
...
| `evidence.py` | 離線核准證據格式；不在 ASR 請求路徑 |

## 修改導覽

- 路由與執行邊界：[`docs/asr/framework.md`](../../../../docs/asr/framework.md)
- 設定：[`docs/asr/config-schema.md`](../../../../docs/asr/config-schema.md)
- SageMaker I/O：[`docs/asr/sagemaker-inference-contract.md`](../../../../docs/asr/sagemaker-inference-contract.md)
...
- Lambda remote-only；不得加入或載入模型推論框架。
- Lambda 不做程序內 admission／併發排隊；容量由 SageMaker endpoint 管理。
- `ASR_CONFIG_JSON` 是唯一 ASR 設定來源；解析錯誤不退回預設值。
- 未核准 route/provider 不外呼；Formo prompt 固定在每腔 endpoint 部署設定。
