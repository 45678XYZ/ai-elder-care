"""由相對時間表達與謂語推出事件身分，供 Task 4 驗收與問題排查。
    ...
    python -m scripts.resolve_event_identity "昨天晚上吃了血壓藥" \
        --concept UCO.BehavioralRecord.MedicationBehavior.ScheduledMedication \
...
DEFAULT_CONCEPT = "UCO.BehavioralRecord.MedicationBehavior.ScheduledMedication"


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="推導 canonical 事件身分")
    parser.add_argument("expression", help="原始時間表達，如「昨天晚上」")
    parser.add_argument("--concept", default=DEFAULT_CONCEPT, help="細分類節點 concept_id")
    ...
    predicate = normalize_predicate(args.concept, args.predicate, lexicon, taxonomy)
    ...
    print(f"concept_id           : {args.concept}")
    print(f"high level type      : {taxonomy.high_level_type(args.concept)}")
    print(f"subject              : {args.subject} -> {subject}")
    print(
        "predicate            : "
        f"{args.predicate} -> {predicate.value}"
        f"（命中={predicate.matched} 別名={predicate.via_alias} 來源={predicate.matched_concept_id}）"
    )
