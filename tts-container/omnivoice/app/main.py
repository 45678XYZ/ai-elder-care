...
from .config import OMNIVOICE_SAMPLE_RATE, ContainerConfig, resolve_speaker_dir
    ...
    async def invocations(request: Request) -> Response:
        ...
        try:
            ...
            waveform = await run_in_threadpool(
                state["synthesizer"].synthesize, parsed.text, parsed.dialect, speaker_dir
            )
            audio = await run_in_threadpool(encode_mp3, waveform, OMNIVOICE_SAMPLE_RATE)
        except (SynthesisError, AudioEncodeError):
            # 只記錄輸入長度與耗時；文字、音訊與腔調都不進 log。
            logger.error(
